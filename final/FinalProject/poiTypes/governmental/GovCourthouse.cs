...
public class GovCourthouse : GovernmentalPOI
{
    private List<Person> guards = new List<Person>();
    private Random random = new Random();

    public GovCourthouse(string name, Person owner, int tier, PersonGenerator gen) : base(name, owner, tier)
    {
        int guardCount = random.Next(2, tier + 3);
    ...
    public override List<string> DisplayPOI()
    {
        ...
        returnString.Add($"Courthouse: {GetName()}");
